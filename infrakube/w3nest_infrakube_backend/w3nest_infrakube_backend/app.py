...
from w3nest_infrakube_backend import __version__
from w3nest_infrakube_backend.environment import Configuration, Environment
from w3nest_infrakube_backend.router import router as root_router
...
def create_app(configuration: Configuration) -> FastAPI:
    ...
    app: FastAPI = FastAPI(
        title="w3nest_infrakube_backend",
        # Root path is always served from localhost (using the w3nest server).
        root_path=f"{root_base}:{configuration.host_port}/backends/w3nest_infrakube_backend/{__version__}",
        lifespan=lifespan,
    )
